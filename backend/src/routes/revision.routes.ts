...
export const createRevisionRouter = (options: CreateRevisionRouterOptions = {}): Router => {
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      res.status(200).json(
        revisionService.submitReview(req.authUser!.userId, req.params.topicId, req.body.recallQuality as 1 | 2 | 3 | 4 | 5),
      );
    }),
  );
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      res.status(201).json(revisionService.generateMicroNotes(req.authUser!.userId, req.body.topicId, req.body.sourceContent));
    }),
  );
